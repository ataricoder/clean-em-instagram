...
# get a session!
session = InstaPy(username='clean.em', password='haye12345',disable_image_load=True,  bypass_suspicious_attempt=True, headless_browser=True, nogui=True)
...
with smart_run(session):
    ...
    session.set_comments(["Beautiful place! @{}","Aesthetic photo!"], media='Photo')
    session.set_skip_users(skip_no_profile_pic=True,
                       no_profile_pic_percentage=100)
    session.set_delimit_commenting(enabled=True, comments_mandatory_words=['home', 'house', 'apartment', 'airbnb', 'condo', 'department'])
    ...
    session.follow_by_tags(['austinluxuryhomes'], amount=1000, interact=True)
